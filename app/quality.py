...
DEFAULT_POLICY = {
    'scope': 'overall', 'max_blockers': 0, 'min_coverage': None,
...
def validate_policy(value):
    ...
    for key in ('min_coverage', 'max_duplication'):
        v = policy[key]
    ...
    for key in ('cpd', 'require_hotspot_review', 'store_source'):
    ...
    if any(not re.fullmatch(r'[A-Za-z][A-Za-z0-9_]{0,99}', v) for v in policy['disabled_rules']):
        raise ValueError('Invalid rule name.')
    overrides = policy['severity_overrides']
    if not isinstance(overrides, dict) or len(overrides) > 100 or any(not re.fullmatch(r'[A-Za-z][A-Za-z0-9_]{0,99}', k) or v not in SEVERITIES for k, v in overrides.items()):
...
def analysis_signature(policy):
    return hashlib.sha256(json.dumps({k: policy[k] for k in ('categories', 'exclusions', 'disabled_rules')}, sort_keys=True).encode()).hexdigest()
